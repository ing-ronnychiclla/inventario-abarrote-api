...
public interface LoteRepository extends JpaRepository<Lote, UUID> {
    ...
    @Query("SELECT COALESCE(SUM(l.cantidadActual), 0) FROM Lote l WHERE l.producto.id = :productoId")
    Integer sumCantidadActualByProductoId(@Param("productoId") UUID productoId);

    @Query("SELECT l.producto.id, COALESCE(SUM(l.cantidadActual), 0) FROM Lote l GROUP BY l.producto.id")
    List<Object[]> obtenerStockDisponiblePorProducto();
}
